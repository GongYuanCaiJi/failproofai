...
ユニットテストは `__tests__/` に配置されており、`jsdom` を使った [Vitest](https://vitest.dev) で実行されます。
...
__tests__/e2e/
  ...
  hooks/
    ...
    policy-params.e2e.test.ts      # パラメータ付きポリシーへのパラメータ注入
```
...
マージ前に、CI のフルラン（`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`）がすべてパスする必要があります。E2Eスイートは別の CI ジョブとして並列で実行されます。
