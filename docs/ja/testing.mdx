...
ユニットテストは `__tests__/` ディレクトリに配置されており、`happy-dom` を使った [Vitest](https://vitest.dev) で実行されます。
...
E2Eテストは実際の `failproofai` バイナリをサブプロセスとして起動し、JSONペイロードを stdin に渡して、stdout の出力と終了コードを検証します。これにより、Claude Code が使用する完全な統合パスをテストできます。
...
公開フック API（`src/hooks/custom-hooks-registry.ts`、`src/hooks/policy-helpers.ts`、または `src/hooks/policy-types.ts`）を変更した場合は、その都度 `dist/` を再ビルドしてください。
...
__tests__/e2e/
  helpers/
    hook-runner.ts      # バイナリを起動し、ペイロード JSON を渡して、終了コード・stdout・stderr をキャプチャ
    fixture-env.ts      # テストごとの独立した一時ディレクトリと設定ファイル
...
`forks` プールは重要です。スレッドベースのワーカーは `globalThis` を共有するため、サブプロセスを起動するテストと干渉する可能性があります。プロセスベースの forks はこの問題を回避できます。

---

## CI

マージ前に、フルCIの実行（`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`）がすべてパスしている必要があります。E2Eスイートは別のCIジョブとして並列で実行されます。
