...
Bu komut, `failproofai policies --install` tarafından Claude Code'un `settings.json` dosyasına kayıtlıdır. Normalde bunu doğrudan çağırmazsınız.

stdin'den bir JSON yükü okur, etkinleştirilmiş tüm politikaları değerlendirir ve kararı belirten bir kodla çıkar:
