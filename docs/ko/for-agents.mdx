---
title: "에이전트를 위한 가이드"
description: "한 줄의 명령으로 코딩 에이전트에 Failproof AI 지식을 추가하세요. Claude Code, Cursor, Windsurf 등과 호환됩니다."
---

한 줄의 명령으로 코딩 에이전트에 Failproof AI 전체 레퍼런스를 추가하세요. Claude Code, Cursor, Windsurf, 그리고 스킬을 지원하는 모든 에이전트와 호환됩니다.
...
Mintlify는 네비게이션의 모든 페이지에서 `llms.txt`를 생성합니다. Failproof AI 문서는 전체 API를 포함하며, 모든 정책, 옵션, 예제가 담겨 있습니다. 누락된 내용이 있다면 원본은 `https://docs.befailproof.ai/llms-full.txt`에서 확인할 수 있습니다.

특정 컨텍스트만 필요하다면 특정 페이지를 직접 링크하세요:

```bash
# custom policies API만 추가
