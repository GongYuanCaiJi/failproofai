...
failproofai에는 두 가지 테스트 스위트가 있습니다: **단위 테스트** (빠르고 모킹 기반)와 **엔드투엔드 테스트** (실제 서브프로세스 호출).
...
단위 테스트는 `__tests__/` 디렉토리에 위치하며 [Vitest](https://vitest.dev)와 `happy-dom`을 사용합니다.
...
// env.cwd    - 임시 디렉토리; payload.cwd로 전달하면 .failproofai/policies-config.json을 읽어옴
...
E2E 테스트는 `vitest.config.e2e.mts`를 사용하며 다음과 같이 설정됩니다:

- `environment: "node"` - 브라우저 전역 변수 불필요
- `pool: "forks"` - 진정한 프로세스 격리 (테스트가 서브프로세스를 실행함)
- `testTimeout: 20_000` - 테스트당 20초 (바이너리 시작 + 훅 평가)

`forks` 풀은 중요한 설정입니다. 스레드 기반 워커는 `globalThis`를 공유하므로 서브프로세스를 실행하는 테스트에 영향을 줄 수 있습니다. 프로세스 기반 포크 방식은 이 문제를 방지합니다.
...
완전한 머지 전 체크리스트는 [Contributing](../CONTRIBUTING.md)을 참조하세요.