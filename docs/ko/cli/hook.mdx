...
이것은 `failproofai policies --install`이 Claude Code의 `settings.json`에 등록하는 명령어입니다. 일반적으로 직접 호출할 필요가 없습니다.

stdin에서 JSON 페이로드를 읽고, 활성화된 모든 정책을 평가한 후, 결정 내용을 나타내는 종료 코드와 함께 종료합니다:

| 종료 코드 | 결정 | 효과 |
|-----------|----------|--------|
| `0` | `allow` | 작업 허용 |
| `1` | `deny` | 작업 차단 - Claude가 거부 사유를 확인함 |
| `2` | `instruct` | Claude의 컨텍스트에 지침 삽입 |
