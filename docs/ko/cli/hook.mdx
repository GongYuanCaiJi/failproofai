...
이 명령은 `failproofai policies --install`이 Claude Code의 `settings.json`에 등록하는 명령입니다. 일반적으로 직접 호출할 필요가 없습니다.

stdin에서 JSON 페이로드를 읽고 활성화된 모든 정책을 평가한 후, 결정 결과를 나타내는 종료 코드로 종료합니다:
