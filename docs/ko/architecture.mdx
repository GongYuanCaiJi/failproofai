...
이 문서는 failproofai의 내부 동작 방식을 설명합니다. 훅 시스템이 에이전트 도구 호출을 어떻게 인터셉트하는지, 설정이 어떻게 로드되고 병합되는지, 정책이 어떻게 평가되는지, 그리고 대시보드가 에이전트 활동을 어떻게 모니터링하는지 다룹니다.
...
두 서브시스템은 `~/.failproofai/` 및 프로젝트의 `.failproofai/` 디렉토리에 있는 설정 파일을 공유하지만, 별도의 프로세스로 실행되며 파일시스템을 통해서만 통신합니다.
...
그러면 Claude Code는 각 도구 호출 전에 `failproofai --hook PreToolUse`를 서브프로세스로 실행하며, stdin으로 JSON 페이로드를 전달합니다.
...
`PostToolUse` 이벤트의 경우, 페이로드에 도구의 출력 결과인 `tool_result`도 포함됩니다.

핸들러는 stdin 입력을 1MB로 제한합니다. 이를 초과하는 페이로드는 무시되며, 모든 정책이 암묵적으로 allow됩니다.
...
`allow(message)`를 사용하면 작업이 허용될 때도 정책이 Claude에 정보성 컨텍스트를 전달할 수 있습니다. 훅 핸들러는 **stdout**에 다음 JSON을 작성합니다 (설정 파일이 아니라, deny 및 instruct 응답과 동일하게 Claude Code에 대한 핸들러의 응답입니다):
...
```
- 종료 코드: `0` (작업 허용됨)
- 여러 정책이 메시지와 함께 `allow`를 반환하는 경우, 메시지들은 줄바꿈으로 연결되어 단일 `additionalContext` 문자열로 합쳐짐
...
LLM 호출 없이 일반적인 페이로드는 100ms 이내에 전체 처리가 완료됩니다.
...
- `policyParams` - 정책별 키, 해당 값을 정의한 첫 번째 파일이 전체적으로 우선
...
- 그 외의 경우 allow 응답을 출력합니다 (stdout 비어 있음, 종료 코드 0).

---

## 내장 정책

`src/hooks/builtin-policies.ts`는 `BuiltinPolicyDefinition` 객체로 26개의 내장 정책을 정의합니다:
...
`params`를 허용하는 정책은 각 파라미터의 타입과 기본값이 포함된 `PolicyParamsSchema`를 선언합니다. 정책 평가기는 `fn` 호출 전에 해결된 값을 `ctx.params`에 주입합니다. 기본값이 항상 먼저 적용되므로, 정책 함수는 null 검사 없이 `ctx.params`를 읽습니다.

정책 내부의 패턴 매칭은 원시 문자열 매칭이 아닌 파싱된 명령 토큰(argv)을 사용합니다. 이를 통해 셸 연산자 주입을 통한 우회를 방지합니다 (예: `sudo systemctl status *` 패턴은 명령에 `; rm -rf /`를 추가해도 우회되지 않습니다).
...
4. ESM 호환성을 위해 재귀적으로 전이적 로컬 임포트를 재작성합니다.
5. 임시 `.mjs` 파일을 작성하고 엔트리 파일을 `import()`합니다.
6. `getCustomHooks()`를 호출하여 등록된 훅을 가져옵니다.
7. `finally` 블록에서 모든 임시 파일을 정리합니다.

오류 발생 시 (파일 없음, 문법 오류, 임포트 실패), 오류는 `~/.failproofai/hook.log`에 기록되고 로더는 빈 배열을 반환합니다. 내장 정책에는 영향을 미치지 않습니다.

커스텀 정책은 모든 내장 정책 이후에 평가됩니다. 커스텀 정책의 `deny`는 이후 커스텀 정책의 단락 평가를 발생시키지만, 이 시점에서 모든 내장 정책은 이미 실행된 상태입니다.

---

## 활동 로깅

각 훅 이벤트 후, 핸들러는 `~/.failproofai/hook-activity.jsonl`에 JSONL 라인을 추가합니다:
...
allow가 아닌 결정을 내린 정책당 한 줄씩 기록됩니다. allow 결정은 파일 크기를 줄이기 위해 기록하지 않습니다.

---

## 대시보드 아키텍처

대시보드는 React Server Components와 Server Actions를 사용하는 App Router 기반의 **Next.js 16** 애플리케이션입니다.
...
app/
  ...
  api/
    download/[project]/[session]/route.ts   ← 세션별 내보내기 (JSONL 또는 JSON)
```
...
- 읽기 페이지에 React Server Components 사용 - 초기 로딩 속도가 빠르고 데이터 패칭을 위한 클라이언트 번들이 없습니다.
