...
일반적인 시나리오에 바로 사용할 수 있는 예시들입니다. 각 예시는 설치 방법과 예상 동작을 보여줍니다.
...
가장 흔한 설정 — 에이전트가 되돌릴 수 없는 피해를 입히는 것을 방지합니다.
...
에이전트가 도구 출력에서 자격 증명을 보거나 유출하는 것을 막습니다.
...
## 에이전트가 응답을 기다릴 때 Slack 알림 받기

알림 훅을 사용해 유휴 알림을 Slack으로 전달하세요.
...
에이전트에게 커밋하기 전에 테스트를 실행하도록 알립니다.
...
프로젝트 레벨 설정을 커밋해 팀 모든 개발자가 동일한 정책을 적용받도록 합니다.
...
failproofai를 설치한 모든 팀원에게 이 규칙이 자동으로 적용됩니다.

---

## 컨벤션 정책으로 조직 전체 품질 기준 구축

가장 효과적인 설정: 프로젝트에 맞게 작성된 정책을 `.failproofai/policies/` 디렉토리에 담아 저장소에 커밋하세요. 모든 팀원이 자동으로 적용받습니다 — 별도의 설치 명령이나 설정 변경이 필요 없습니다.
...
<Steps>
  ...
  <Step title="지속적으로 개선">
    팀에서 새로운 문제가 발생할 때마다 정책을 추가하고 푸시하세요. 다음 `git pull` 시 모든 팀원이 업데이트를 받습니다. 이 정책들은 팀과 함께 성장하는 살아있는 품질 기준이 됩니다.
  </Step>
</Steps>
