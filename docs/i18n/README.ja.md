...
インシデントになる前に検出・阻止します。レイテンシゼロ。ローカルで動作。
...
エージェントが行うすべてのツール呼び出しはローカルに記録されます。ダッシュボードでは、実行された内容、ブロックされた内容、ポリシーがエージェントに伝えた内容を確認できます。問題が発生したとき、原因を推測する必要はありません。→ [ダッシュボードガイド](https://docs.befailproof.ai/dashboard)
