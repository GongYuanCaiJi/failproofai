...
**Giải quyết lỗi runtime cho các tác nhân mã hóa.**
...
> Cài đặt hooks cho một hoặc nhiều CLI: `failproofai policies --install --cli opencode pi gemini` (hoặc `--cli claude codex copilot cursor opencode pi gemini`). Bỏ qua `--cli` để tự động phát hiện các CLI được cài đặt và nhắc. **Hỗ trợ GitHub Copilot CLI, Cursor Agent, OpenCode, Pi và Gemini CLI đang trong giai đoạn beta — quá trình kiểm thử đang diễn ra.**
...
| `sanitize-api-keys` | Khóa API rò rỉ vào bối cảnh tác nhân |
...
Cam kết nó và toàn bộ nhóm nhận được nó khi pull tiếp theo.
...
| `deny(message)` | Chặn nó — tin nhắn được gửi lại đến tác nhân |
| `instruct(message)` | Cho phép nó qua, nhưng thêm bối cảnh vào lời nhắc tiếp theo của tác nhân |
...
Mỗi lệnh gọi công cụ mà tác nhân của bạn thực hiện đều được ghi nhật ký cục bộ. Bảng điều khiển cho thấy những gì đã chạy,
những gì bị chặn và những gì chính sách đã nói với tác nhân — vì vậy bạn không phải đoán
...
| [Chính sách tích hợp sẵn](https://docs.befailproof.ai/built-in-policies) | Tất cả 30 chính sách với các tham số |
...
Xem [CONTRIBUTING.md](./CONTRIBUTING.md). Chính sách mới, trường hợp biên và bản dịch đều được hoan nghênh.
