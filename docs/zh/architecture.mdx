...
本文档介绍 failproofai 的内部工作机制：钩子系统如何拦截 Agent 工具调用、配置如何加载与合并、策略如何评估，以及 Dashboard 如何监控 Agent 活动。

---

## 概述

failproofai 由两个独立子系统组成：
...
两个子系统共享 `~/.failproofai/` 和项目 `.failproofai/` 目录中的配置文件，但它们作为独立进程运行，仅通过文件系统通信。
...
运行 `failproofai policies --install` 后，它会在 `~/.claude/settings.json` 中写入如下条目：
...
Claude Code 随后会在每次工具调用前将 `failproofai --hook PreToolUse` 作为子进程调用，并通过 stdin 传入 JSON 载荷。
...
对于 `PostToolUse` 事件，载荷还包含带有工具输出的 `tool_result` 字段。

处理器强制限制 stdin 为 1 MB。超过此限制的载荷将被丢弃，所有策略隐式允许。
...
`allow(message)` 允许策略在操作被允许的情况下仍向 Claude 发送信息性上下文。钩子处理器将以下 JSON 写入 **stdout**（这不是配置文件——这是处理器对 Claude Code 的响应，与 deny 和 instruct 响应方式相同）：
...
```
- 退出码：`0`（操作被允许）
- 当多个策略返回带消息的 `allow` 时，其消息将以换行符连接，合并为一个 `additionalContext` 字符串
...
对于典型载荷，整个过程在 100ms 内完成，无需任何 LLM 调用。
...
合并逻辑：
- `enabledPolicies` - 对三个文件做去重并集
- `policyParams` - 按策略名为键，第一个定义该键的文件完全优先
- `customPoliciesPath` - 第一个定义它的文件优先
- `llm` - 第一个定义它的文件优先

Web Dashboard 使用 `readHooksConfig()`（仅全局配置）进行读写，因为它不以项目 cwd 启动。
...
所有策略执行完毕后：
- 若有任意 `deny` 返回，输出拒绝响应。
- 若收集到任意 `instruct` 返回，输出单条指令响应，所有消息合并后输出。
...
接受 `params` 的策略会声明一个 `PolicyParamsSchema`，为每个参数定义类型和默认值。策略评估器在调用 `fn` 前将解析后的值注入到 `ctx.params` 中。策略函数读取 `ctx.params` 时无需空值守卫，因为默认值始终已提前应用。

策略内部的模式匹配使用解析后的命令 token（argv），而非原始字符串匹配。这可防止通过 Shell 运算符注入绕过（例如，`sudo systemctl status *` 的匹配模式无法通过在命令末尾追加 `; rm -rf /` 来绕过）。
...
发生任何错误（文件未找到、语法错误、导入失败）时，错误会记录到 `~/.failproofai/hook.log`，加载器返回空数组。内置策略不受影响。

自定义策略在所有内置策略之后执行。自定义策略的 `deny` 仍会短路后续自定义策略（但此时所有内置策略均已执行完毕）。

---

## 活动日志

每次钩子事件处理完成后，处理器会向 `~/.failproofai/hook-activity.jsonl` 追加一行 JSONL：
...
每行对应一条做出非允许决策的策略。允许决策不记录日志（以保持文件精简）。

---

## Dashboard 架构

Dashboard 是一个 **Next.js 16** 应用，采用 App Router，结合 React Server Components 和 Server Actions。
...
app/
  ...
  actions/
    ...
    update-hooks-config.ts    ← 切换策略开/关
...
- 策略页面使用 Server Actions 处理所有变更（切换、参数更新、安装/移除）。
- 会话查看器解析 Claude 的 JSONL 转录格式，并以时间线形式渲染消息和工具调用。

**关键设计决策：**

- 无数据库——所有持久化状态存储在纯文件中（`~/.failproofai/`、`~/.claude/projects/`）。
...
│   ├── hook-activity-store.ts    # 活动持久化到 hook-activity.jsonl
