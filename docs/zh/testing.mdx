...
E2E 测试会将真实的 `failproofai` 二进制文件作为子进程调用，通过 stdin 传入 JSON 载荷，并对 stdout 输出和退出码进行断言。这会测试 Claude Code 所使用的完整集成路径。
...
__tests__/e2e/
  helpers/
    ...
    payloads.ts         # 针对各事件类型的 Claude 准确载荷工厂函数
  hooks/
...
| Stop instruct | `2` | 空 stdout；原因写入 stderr |
| allow | `0` | 空字符串 |
...
- `testTimeout: 20_000` - 每个测试 20 秒超时（二进制启动 + hook 评估）

`forks` 池非常重要：基于线程的 worker 共享 `globalThis`，可能会干扰子进程启动类测试。基于进程的 forks 可以避免这一问题。

---

## CI

在合并前，必须通过完整的 CI 运行（`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`）。E2E 套件作为独立的 CI 任务并行运行。
