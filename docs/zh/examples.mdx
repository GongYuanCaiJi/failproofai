...
适用于常见场景的即开即用示例，每个示例均展示安装方式及预期效果。

---

## 为 Claude Code 配置钩子

Failproof AI 通过 Claude Code 的[钩子系统](https://docs.anthropic.com/en/docs/claude-code/hooks)与其集成。运行 `failproofai policies --install` 后，它会在 Claude Code 的 `settings.json` 中注册钩子命令，并在每次工具调用时触发。
...
如果你正在使用 [Agents SDK](https://docs.anthropic.com/en/docs/agents-sdk) 进行开发，可以通过编程方式使用同一套钩子系统。
...
在项目中提交一份项目级配置文件，让团队所有成员共享相同的策略。
...
最具价值的使用方式：将 `.failproofai/policies/` 连同针对项目定制的策略一起提交到仓库。每位团队成员都能自动获取——无需执行安装命令，无需修改配置。
...
<Steps>
  <Step title="创建目录并添加策略">
    ...
    // 强制使用团队指定的包管理器
    // （也可以直接启用内置的 prefer-package-manager 策略）
    customPolicies.add({
      name: "enforce-bun",
      match: { events: ["PreToolUse"] },
      fn: async (ctx) => {
  ...
  <Step title="持续改进">
    每当团队遇到新的问题模式，只需添加策略并推送。所有人在下次 `git pull` 时即可获得更新。这些策略将成为随团队共同成长的活文档质量标准。
  </Step>
</Steps>
...
| `policies-advanced/index.js` | 传递性导入、异步钩子、PostToolUse 输出脱敏、Stop 事件处理 |