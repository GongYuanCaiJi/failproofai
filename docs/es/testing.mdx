...
failproofai tiene dos suites de pruebas: **pruebas unitarias** (rápidas, con mocks) y **pruebas de extremo a extremo** (invocaciones reales de subprocesos).
...
Las pruebas E2E invocan el binario real de `failproofai` como subproceso, envían un payload JSON por stdin y verifican la salida en stdout junto con el código de salida. Esto prueba la ruta de integración completa que utiliza Claude Code.
...
__tests__/e2e/
  helpers/
    ...
    payloads.ts         # Fábricas de payloads precisos para cada tipo de evento
  hooks/
...
// env.home   - directorio home aislado; evita filtraciones del ~/.failproofai real
...
| Stop instruct | `2` | stdout vacío; razón en stderr |
...
- `testTimeout: 20_000` - 20s por prueba (inicio del binario + evaluación del hook)
...
La ejecución completa de CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) debe pasar antes de fusionar cualquier cambio. La suite E2E se ejecuta como un job de CI separado en paralelo.
