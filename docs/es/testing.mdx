...
### Cómo escribir una prueba unitaria de políticas
...
Las pruebas E2E invocan el binario real de `failproofai` como subproceso, envían un payload JSON a stdin y verifican la salida en stdout junto con el código de salida. Esto prueba la ruta de integración completa que utiliza Claude Code.
...
__tests__/e2e/
  helpers/
    ...
    payloads.ts         # Fábricas de payloads precisas de Claude para cada tipo de evento
  hooks/
    ...
    config-scopes.e2e.test.ts      # Combinación de configuración entre scopes project/local/global
...
La ejecución completa de CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) debe completarse sin errores antes de fusionar cualquier cambio. La suite E2E se ejecuta como un trabajo de CI independiente en paralelo.
