...
El manejador aplica un límite de 1 MB en stdin. Los payloads que superen este límite se descartan y todas las políticas permiten implícitamente la operación.
...
- Cuando varias políticas devuelven `allow` con mensaje, sus mensajes se unen con saltos de línea en una única cadena `additionalContext`
- Si ninguna política proporciona un mensaje, stdout está vacío (igual que antes)

### Flujo de procesamiento

`src/hooks/handler.ts` implementa el flujo completo:
...
El proceso completo se ejecuta en menos de 100 ms para payloads típicos, sin ninguna llamada a LLM.
...
- `policyParams` - por clave de política, gana íntegramente el primer archivo que la define
- `customPoliciesPath` - gana el primer archivo que lo define
...
1. Se busca el esquema de `params` de la política (si tiene uno).
2. Se lee `policyParams[policy.name]` de la configuración combinada.
3. Se combinan los valores proporcionados por el usuario sobre los valores predeterminados del esquema para producir `ctx.params`.
...
- Si se devolvió algún `deny`, se emite la respuesta de denegación.
- Si se recopilaron respuestas `instruct`, se emite una única respuesta de instrucción con todos los mensajes unidos.
- En caso contrario, se emite una respuesta de permiso (stdout vacío, exit 0).
...
La coincidencia de patrones dentro de las políticas utiliza tokens de comando analizados (argv), no coincidencia de cadenas sin procesar. Esto evita la evasión mediante inyección de operadores de shell (por ejemplo, un patrón para `sudo systemctl status *` no puede eludirse añadiendo `; rm -rf /` al comando).
...
Las políticas personalizadas se evalúan después de todas las políticas integradas. Un `deny` de una política personalizada sigue cortocircuitando las políticas personalizadas posteriores (pero todas las integradas ya han sido ejecutadas en ese momento).
...
app/
  ...
  project/[name]/page.tsx     ← Componente servidor: lista las sesiones de un proyecto
...
- El visor de sesión analiza el formato de transcripción JSONL de Claude y renderiza una línea temporal de mensajes y llamadas a herramientas.

**Decisiones de diseño clave:**

- Sin base de datos - todo el estado persistente se almacena en archivos planos (`~/.failproofai/`, `~/.claude/projects/`).
...
- Componentes cliente únicamente donde se necesita interactividad (activación de políticas, búsqueda de actividad, visor de registros).
...
├── src/hooks/
│   ├── handler.ts                # Flujo de eventos de hook
...
│   ├── hooks-config.ts           # Carga de configuración multinivel
...
│   ├── manager.ts                # Operaciones de install / remove / list
│   ├── install-prompt.ts         # Prompt interactivo de selección de políticas
│   ├── hook-logger.ts            # Registro en hook.log
│   ├── hook-activity-store.ts    # Persistencia de actividad en hook-activity.jsonl
