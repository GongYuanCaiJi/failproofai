...
El dashboard de failproofai es una aplicación web local para monitorear las sesiones de tus agentes de IA y gestionar políticas. Descubre qué hicieron tus agentes mientras no estabas.
...
- Recuento de actividad de hooks (políticas que se ejecutaron)
...
- **Actividad de políticas** — Para cada llamada a herramienta, qué políticas se ejecutaron y qué decisión devolvieron
...
<Tabs>
  ...
  - Historial paginado completo de cada evento de hook que se ha ejecutado en todas las sesiones
...
Si solo necesitas algunas partes del dashboard, define `FAILPROOFAI_DISABLE_PAGES` con una lista separada por comas de nombres de páginas:
...
El dashboard admite modo claro y modo oscuro. Cámbialo mediante el botón en la barra de navegación. La preferencia se almacena en el almacenamiento local de tu navegador.

---

## Configurar la ruta de proyectos

De forma predeterminada, el dashboard lee desde el directorio estándar de proyectos de Claude Code. Puedes sobrescribirlo para configuraciones personalizadas:
...
Cuando ejecutas el dashboard en **modo dev** (`npm run dev`) y accedes a él desde un hostname distinto de `localhost` — por ejemplo, un dominio personalizado, una IP remota o una URL tunelizada — puede que veas una advertencia como:

```text
⚠ Blocked cross-origin request to Next.js dev resource /_next/webpack-hmr from "dashboard.example.com".
```

Esto ocurre porque Next.js bloquea el acceso de origen cruzado a su websocket HMR (recarga en caliente de módulos), que es una función exclusiva del modo de desarrollo. Para permitir tu host, usa el flag `--allowed-origins`:
...
También puedes definir la variable de entorno `FAILPROOFAI_ALLOWED_DEV_ORIGINS` en su lugar:
