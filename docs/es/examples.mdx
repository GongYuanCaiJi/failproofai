...
Failproof AI se integra con Claude Code a través de su [sistema de hooks](https://docs.anthropic.com/en/docs/claude-code/hooks). Cuando ejecutas `failproofai policies --install`, registra comandos de hook en el archivo `settings.json` de Claude Code que se disparan en cada llamada a una herramienta.
...
<Steps>
  ...
  <Step title="Activar todas las políticas integradas">
    ```bash
...
<Steps>
  ...
  <Step title="Configurar hooks en tu agente">
    Pasa comandos de hook al crear el proceso de tu agente. Los hooks se disparan de la misma manera que en Claude Code — mediante JSON por stdin/stdout:
...
La configuración más habitual: evita que los agentes causen daños irreversibles.
...
- `block-curl-pipe-sh` — bloquea la canalización de scripts remotos al shell
...
Estas se disparan en `PostToolUse` — después de que una herramienta se ejecuta, limpian la salida antes de que el agente la vea.
...
## Establecer un estándar de calidad organizacional con políticas de convención

La configuración con mayor impacto: confirma `.failproofai/policies/` en tu repositorio con políticas adaptadas a tu proyecto. Todos los miembros del equipo las recibirán automáticamente — sin comandos de instalación ni cambios de configuración.
...
<Steps>
  ...
  <Step title="Seguir mejorando">
    A medida que tu equipo encuentre nuevos casos de fallo, agrega políticas y súbelas. Todos recibirán la actualización en su próximo `git pull`. Estas políticas se convierten en un estándar de calidad vivo que crece junto con tu equipo.
  </Step>
</Steps>
...
El directorio [`examples/`](https://github.com/failproofai/failproofai/tree/main/examples) en el repositorio contiene:

| Archivo | Qué muestra |
|---------|-------------|
| `policies-basic.js` | Políticas de inicio — bloquear escrituras en producción, force-push y scripts canalizados |
