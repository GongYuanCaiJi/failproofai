...
Failproof AI se integra con Claude Code a través de su [sistema de hooks](https://docs.anthropic.com/en/docs/claude-code/hooks). Al ejecutar `failproofai policies --install`, registra comandos de hook en el archivo `settings.json` de Claude Code que se activan en cada llamada a una herramienta.
...
Estas se activan en `PostToolUse` — después de que se ejecuta una herramienta, limpian la salida antes de que el agente la vea.
...
Incluye una configuración a nivel de proyecto para que todos los desarrolladores de tu equipo utilicen las mismas políticas.
...
Cualquier miembro del equipo que tenga failproofai instalado aplicará estas reglas automáticamente.

---

## Establecer un estándar de calidad para toda la organización con políticas de convención

La configuración más impactante: incluye `.failproofai/policies/` en tu repositorio con políticas adaptadas a tu proyecto. Todos los miembros del equipo las reciben automáticamente — sin comandos de instalación ni cambios de configuración.
...
<Steps>
  <Step title="Crear el directorio y añadir políticas">
    ...
    // Recuerda al agente que ejecute las pruebas antes de hacer commit
    customPolicies.add({
      name: "test-before-commit",
      match: { events: ["PreToolUse"] },
      fn: async (ctx) => {
  ...
  <Step title="Seguir mejorando">
    A medida que tu equipo detecte nuevos problemas, añade políticas y haz push. Todos reciben la actualización en su próximo `git pull`. Estas políticas se convierten en un estándar de calidad vivo que crece con tu equipo.
  </Step>
</Steps>
