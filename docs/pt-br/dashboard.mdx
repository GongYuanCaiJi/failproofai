...
Clique no botão **Download Logs** para exportar a sessão. Para sessões do Claude Code, Codex, Copilot, Cursor, Pi e Gemini, você obtém a transcrição JSONL original do disco byte a byte; para OpenCode (cujas sessões residem no SQLite, não no disco) você obtém um documento JSON espelhando as tabelas subjacentes `session` / `messages` / `parts`.
...
<Tabs>
  ...
  - Histórico completo e paginado de cada evento de hook acionado em todas as sessões
...
Por padrão, o dashboard lê do diretório de projetos padrão do Claude Code. Substitua-o para configurações personalizadas:
...
Isso ocorre porque o Next.js está bloqueando o acesso de origem cruzada ao seu websocket HMR (hot module reload), que é um recurso exclusivo do modo dev. Para permitir seu host, use a flag `--allowed-origins`:
...
<Note>
Isso se aplica apenas ao modo dev. Ao executar `failproofai` (modo de produção), não há websocket HMR nem problema de recurso dev de origem cruzada.
</Note>