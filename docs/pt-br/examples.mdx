...
O Failproof AI integra-se ao Claude Code por meio do seu [sistema de hooks](https://docs.anthropic.com/en/docs/claude-code/hooks). Quando você executa `failproofai policies --install`, ele registra comandos de hook no `settings.json` do Claude Code que disparam a cada chamada de ferramenta.
...
<Steps>
  ...
  <Step title="Configure os hooks no seu agente">
    Passe os comandos de hook ao criar o processo do seu agente. Os hooks disparam da mesma forma que no Claude Code — via JSON pelo stdin/stdout:
...
Faça o commit de uma configuração no nível do projeto para que todos os desenvolvedores do time usem as mesmas políticas.
...
Todos os membros do time que tiverem o failproofai instalado receberão essas regras automaticamente.

---

## Construa um padrão de qualidade para toda a organização com políticas de convenção

A configuração de maior impacto: faça o commit de `.failproofai/policies/` no seu repositório com políticas adaptadas ao seu projeto. Todos os membros do time as recebem automaticamente — sem comandos de instalação, sem alterações de configuração.
...
<Steps>
  ...
  <Step title="Faça o commit no git">
    ```bash
  ...
  <Step title="Continue evoluindo">
    À medida que seu time identifica novos tipos de falha, adicione políticas e faça o push. Todos recebem a atualização no próximo `git pull`. Essas políticas se tornam um padrão de qualidade vivo que cresce junto com o time.
  </Step>
</Steps>
...
|---------|-----------------|
| `policies-basic.js` | Políticas iniciais — bloqueia escrita em produção, force-push e scripts redirecionados |
| `policies-notification.js` | Alertas no Slack para notificações de inatividade e fim de sessão |
