...
1. **Manipulador de hooks** - Um subprocesso de CLI rápido que o Claude Code invoca a cada chamada de ferramenta do agente. Avalia as políticas e retorna uma decisão.
...
O manipulador aplica um limite de 1 MB no stdin. Payloads que excedam esse limite são descartados e todas as políticas permitem implicitamente.
...
1. Busca o esquema `params` da política (se houver).
...
A correspondência de padrões dentro das políticas usa tokens de comando analisados (argv), não correspondência de strings brutas. Isso impede bypass via injeção de operadores de shell (por exemplo, um padrão para `sudo systemctl status *` não pode ser contornado adicionando `; rm -rf /` ao comando).
...
As políticas personalizadas são avaliadas após todas as políticas integradas. Um `deny` de uma política personalizada ainda interrompe as demais políticas personalizadas (mas todos os integrados já terão sido executados nesse ponto).
...
app/
  ...
  actions/
    ...
    get-hook-activity.ts      ← Paginação/busca no log de atividades
...
- A página de Políticas usa Server Actions para todas as mutações (ativar/desativar, atualizar parâmetros, instalar/remover).
...
- Client components apenas onde há necessidade de interatividade (alternância de políticas, busca de atividades, visualizador de logs).
...
failproofai/
├── bin/
│   └── failproofai.mjs           # Roteador de CLI (hook / dashboard / install / etc.)
├── src/hooks/
...
│   ├── hooks-config.ts           # Carregamento de config em múltiplos escopos
...
│   └── ...
├── components/                   # Componentes de UI React compartilhados
