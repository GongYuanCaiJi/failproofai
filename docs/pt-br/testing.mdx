...
# Executa os testes E2E (requer configuração - veja abaixo)
bun run test:e2e
...
Os testes unitários ficam em `__tests__/` e usam [Vitest](https://vitest.dev) com `happy-dom`.
...
__tests__/
  hooks/
    ...
    custom-hooks-loader.test.ts   # ESM loader, imports transitivos, tratamento de erros
...
__tests__/e2e/
  ...
  hooks/
    ...
    config-scopes.e2e.test.ts      # Mesclagem de configs entre escopos project/local/global
...
// env.home   - home dir isolado; sem vazamentos do ~/.failproofai real
...
- `environment: "node"` - não são necessários globals de browser
