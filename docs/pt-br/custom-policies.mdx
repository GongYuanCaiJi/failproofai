...
description: "Escreva suas próprias políticas em JavaScript - imponha convenções, previna desvios, detecte falhas e integre com sistemas externos"
icon: code
---

As políticas personalizadas permitem que você escreva regras para qualquer comportamento do agente: impor convenções de projeto, prevenir desvios, controlar operações destrutivas, detectar agentes travados ou integrar com Slack, fluxos de aprovação e muito mais. Elas utilizam o mesmo sistema de eventos de hook e as decisões `allow`, `deny`, `instruct` das políticas integradas.
...
Coloque arquivos `*policies.{js,mjs,ts}` na pasta `.failproofai/policies/` e eles serão carregados automaticamente — sem necessidade de flags ou alterações de configuração. Funciona como git hooks: basta colocar o arquivo e pronto.
...
**Como funciona:**
- Os diretórios de projeto e de usuário são escaneados (união — não "primeiro escopo vence")
...
<Tip>
As políticas por convenção são a forma mais fácil de criar um padrão de qualidade para sua organização. Faça o commit de `.failproofai/policies/` no git e todos os membros do time receberão as mesmas regras automaticamente — sem necessidade de configuração individual. À medida que o time descobrir novos modos de falha, adicione uma política e faça o push. Com o tempo, essas políticas se tornam um padrão de qualidade vivo que melhora a cada contribuição.
</Tip>
...
O caminho absoluto resolvido é armazenado em `policies-config.json` como `customPoliciesPath`. O arquivo é carregado a cada evento de hook — não há cache entre eventos.
...
`deny(message)` - a mensagem aparece para Claude com o prefixo `"Blocked by failproofai:"`. Um único `deny` interrompe toda a avaliação subsequente.
...
Casos de uso:
- **Confirmações de status:** `allow("All CI checks passed.")` — informa a Claude que tudo está verde
...
2. Políticas personalizadas explícitas do `customPoliciesPath` (em ordem de `.add()`)
...
| Falha | Comportamento |
|-------|---------------|
| `customPoliciesPath` não configurado | Nenhuma política personalizada explícita é executada; políticas de convenção e integradas continuam normalmente |
...
// Previne alterações de dependências não planejadas durante o período de freeze
customPolicies.add({
...
| `examples/convention-policies/workflow-policies.mjs` | Políticas de fluxo de trabalho baseadas em convenção (lembretes de testes, auditoria de escritas de arquivos) |
