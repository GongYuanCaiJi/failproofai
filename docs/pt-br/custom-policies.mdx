...
```
# Nível do projeto — versionado no git, compartilhado com o time
...
**Como funciona:**
- Tanto o diretório do projeto quanto o do usuário são escaneados (união — não vence o primeiro escopo encontrado)
...
<Tip>
Políticas por convenção são a forma mais fácil de construir um padrão de qualidade para sua organização. Versione `.failproofai/policies/` no git e todos os membros do time passam a ter as mesmas regras automaticamente — sem configuração por desenvolvedor. Conforme o time descobre novos modos de falha, adicione uma política e faça o push. Com o tempo, essas políticas se tornam um padrão de qualidade vivo que melhora a cada contribuição.
</Tip>
...
O caminho absoluto resolvido é armazenado em `policies-config.json` como `customPoliciesPath`. O arquivo é recarregado a cada evento de hook — não há cache entre eventos.

### Usando ambas as formas juntas

Políticas por convenção e o arquivo `--custom` explícito podem coexistir. Ordem de carregamento:
...
`deny(message)` — a mensagem é exibida para Claude com o prefixo `"Blocked by failproofai:"`. Um único `deny` interrompe toda a avaliação subsequente.
...
<Tip>
Você pode adicionar orientações extras a qualquer mensagem `deny` ou `instruct` incluindo um campo `hint` em `policyParams` — sem necessidade de alterar o código. Isso funciona para políticas customizadas (`custom/`), de convenção do projeto (`.failproofai-project/`) e de convenção do usuário (`.failproofai-user/`) também. Veja [Configuração → hint](/pt-br/configuration#hint-cross-cutting) para mais detalhes.
</Tip>

### Mensagens informativas em allow
...
- **Explicações de fail-open:** `allow("GitHub CLI not installed, skipping CI check.")` — informa Claude por que uma verificação foi ignorada para que tenha o contexto completo
...
customPolicies.add({
  ...
  match: { events: ["Stop"] },
  fn: async (ctx) => {
    // Só dispara quando a sessão é encerrada
...
Políticas personalizadas são **fail-open**: erros nunca bloqueiam as políticas embutidas nem derrubam o handler do hook.

| Falha | Comportamento |
|---------|----------|
| `customPoliciesPath` não configurado | Nenhuma política personalizada explícita é executada; políticas de convenção e embutidas continuam normalmente |
...
| Diretório de convenção ausente | Nenhuma política de convenção é executada; nenhum erro |
...
// Mantém o agente no caminho certo: verifica testes antes de fazer commit
customPolicies.add({
...
// Previne mudanças não planejadas de dependências durante o período de congelamento
customPolicies.add({
...
| `examples/convention-policies/security-policies.mjs` | Políticas de segurança baseadas em convenção (bloquear escritas em .env, prevenir reescrita do histórico git) |
