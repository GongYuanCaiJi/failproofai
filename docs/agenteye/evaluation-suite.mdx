...
description: "AgentEye Evaluation Suite documentation."
---


AgentEye scores completed agent sessions by POSTing the full event transcript to a
**single customer-owned evaluator service**. The evaluator returns scores either
inline or by handing back a `job_id` for AgentEye to poll. Results are stored and
surfaced in the dashboard.

This guide covers:

1. How session completion is detected.
2. The HTTP contract the evaluator must implement.
3. Configuring the AgentEye server.
4. Viewing results.
5. Troubleshooting.

For the Python helper that implements the contract for you, see the
[`agenteye-evaluator` package on PyPI](https://pypi.org/project/agenteye-evaluator/).
...
The `agenteye-evaluator` Python package gives you a typed FastAPI
wrapper that implements the HTTP contract above. Install it from PyPI:
...
The `app` instance is ASGI-callable, so `uvicorn module:app` runs it.
...
Full API reference, async pattern, and event schema: the `agenteye-evaluator`
README ships inside each release tarball on the
[agenteye-enterprise releases page](https://github.com/agenteye-enterprise/releases),
or you can read it on the package's PyPI page.
...
evaluator container. The release includes reference Kubernetes
manifests under `deploy/examples/evaluator/` that you can apply as-is
after swapping in your image and a shared bearer token.
...
See [deployment.md](/agenteye/deployment) for the full env var table.
...
*Opening a session shows its full timeline alongside the evaluation scores and any dispatcher error in the right rail.*
