...
Get the full AgentEye platform running in your own cloud without operating a single database, certificate, or upgrade yourself. You provide a dedicated Kubernetes cluster, and Exosphere deploys, configures, runs, backs up, and upgrades every component for you, with all data staying inside your cloud account. Choose this path when you want AgentEye's value with none of the day-to-day operational work.

AgentEye is a self-hosted platform for observing, evaluating, and improving your AI agents in production. It captures agent sessions, tool calls, model requests, and errors, turns them into searchable analytics and evaluations, and surfaces the results in a dashboard with an optional AI assistant.

---

## Which deployment guide should I use?

AgentEye runs several ways. Pick the one that matches who you want operating it:

| Guide | Who operates it | Best when |
|---|---|---|
| [Managed deployment](/agenteye/managed-deployment) (this page) | Exosphere, in your cluster | You want the full platform with zero operational work |
| [Getting started](/agenteye/getting-started) | You (Docker Compose) | You want the fastest way to try AgentEye on one machine |
| [Single-pod deployment](/agenteye/single-pod-deployment) | You | You run the collector as a sidecar next to each agent for low-latency, in-pod telemetry |
| [Kubernetes deployment](/agenteye/kubernetes-deployment) | You | You want to deploy and scale every component on your own cluster |

The rest of this page covers the **managed** model, where Exosphere operates everything inside a cluster you own.

---

## Prerequisites

- A **GitHub PAT** for pulling container images and downloading artifacts (see [GitHub token setup](/agenteye/github-token))
...
> **Note:** Exosphere installs and manages everything else inside the cluster: ingress controllers, TLS certificates, databases, caching, monitoring, and all application deployments.
...
> **Note:** Automated certificate issuance and renewal validate over HTTP on port 80 of each load balancer. If your security posture requires restricting the dashboard load balancer to corporate IP ranges, tell Exosphere first. We switch certificate validation to a DNS-based method (one extra DNS record on your side) so renewals keep working behind the restriction.

> **Note:** Cluster nodes need internet access to pull container images from `ghcr.io`. Pulling the private platform images needs signed enterprise access. If your network restricts outbound traffic, allowlist `ghcr.io` or mirror images to your internal registry.
...
Here is how the managed topology fits together. Everything inside the dashed boundary runs in the cluster you own, so your data never leaves your cloud account:

```mermaid
flowchart TD
    AC["Your agent clusters<br/>collectors send events"] -->|"mTLS + API key, 443"| ILB
    BR["Developer browsers<br/>view the dashboard"] -->|"HTTPS, 443"| DLB
    subgraph cloud["Your cloud account (data stays here)"]
        ILB["Ingest LoadBalancer<br/>mTLS ingress"] --> SRV["AgentEye Server"]
        DLB["Dashboard LoadBalancer<br/>HTTPS ingress"] --> DASH["Dashboard (+ AI assistant)"]
        SRV <--> DASH
        SRV --> CH[("ClickHouse")]
        SRV --> PG[("Postgres")]
        SRV --> RDS[("Redis cache")]
        SRV --> BK["Daily backup to<br/>your storage bucket"]
        CM["cert-manager<br/>TLS + mTLS certs"]
    end
```
...
The managed offering also operates the platform's evaluation pipeline, which scores agent activity against your evaluation criteria. See [AI assistant](/agenteye/assistant) and [Evaluation suite](/agenteye/evaluation-suite) for what these capabilities deliver.
...
1. **Install the collector** in each Kubernetes cluster running AI agents: mount the client certificate and configure the endpoint URL and API key. See [Collector installation](/agenteye/collector-installation).
2. **Integrate the Python SDK** into your agent code. See [Python SDK](/agenteye/python-sdk).
