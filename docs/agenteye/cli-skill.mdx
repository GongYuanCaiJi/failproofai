...
Ask your coding agent *"is anything broken today?"* and let it answer from your live AgentEye data, with no commands to memorize. The **AgentEye CLI skill** (`agenteye-cli`) is an *Agent Skill*: a small folder of instructions that a coding agent such as Claude Code or Codex loads on demand. It teaches the agent to operate your AgentEye deployment through the [`agenteye` CLI](/agenteye/cli) from plain-English requests like *"give CI a key that can only push events"* or *"ack the firing incident and assign it to me."*
...
| **[In-dashboard AI assistant](/agenteye/assistant)** | A chat embedded in the dashboard | The agent service (a container in your cluster) | You want in-dashboard Q&A over your data |
...
flowchart TD
    ...
    CLI -->|your authenticated CLI session| API["AgentEye dashboard API"]
```
