...
The skill is maintained alongside the `agenteye` CLI but ships as a **separate folder**, not inside the `pipx install agenteye` package, so don't look for it there. Failproof AI Observability delivers the `agenteye-cli/` folder to you on its own; if you don't have it, ask your Failproof AI contact. Nothing about it is gated: it needs no credential at all, because it only drives the **public** `agenteye` CLI against your own dashboard.
