...
See exactly what your AI agents did in production: every agent run, tool call, model request, hook, and human intervention. The AgentEye Python SDK records that trail from inside your agent code so you can debug, audit, and evaluate what happened. Use it whenever you want AgentEye to observe your agents.
...
> **Tip:** New to AgentEye? Start with [Getting started](/agenteye/getting-started) for the full picture. This page is the complete SDK event reference.
...
In practice you wrap your existing agent code. Bracket a model call with `model_request` before and `model_response` after, so the two events span the real request and AgentEye can pair them:
...
- [Collector installation](/agenteye/collector-installation): install the collector so these events actually ship to AgentEye.
- [Getting started](/agenteye/getting-started): the full setup, from install to seeing your first traces.
- [Troubleshooting](/agenteye/troubleshooting): what to check if events do not appear in the dashboard.
