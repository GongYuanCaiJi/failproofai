...
AgentEye is a self-hosted platform for observing, evaluating, and improving your AI agents in production. This guide is for platform and infrastructure teams who want to run AgentEye at production scale on their own Kubernetes cluster. By the end you'll have a highly available, mutual-TLS-secured deployment: two replicas each of the API server and dashboard, dedicated data stores, and automated daily backups.

Prefer not to operate the cluster yourself? [Managed deployment](/agenteye/managed-deployment) has Exosphere run it for you. Just want a lightweight single-node trial or a collector sidecar? See [Single-pod deployment](/agenteye/single-pod-deployment).

This guide deploys the full AgentEye stack:
...
For the managed deployment model where Exosphere handles all of this on your behalf, see [Managed deployment](/agenteye/managed-deployment).
...
| GitHub PAT | -- | `echo $AGENTEYE_TOKEN` | Non-empty (see [GitHub token setup](/agenteye/github-token)) |
| openssl | -- | `openssl version` | OpenSSL 1.x or 3.x |
| Cloud storage bucket | -- | For PostgreSQL + ClickHouse backups (S3, GCS, or Azure Blob) | -- |

**Cluster sizing:** Minimum 3 nodes, 4 vCPU / 8 GB RAM each. See [Managed deployment](/agenteye/managed-deployment) for full requirements.
...
> **Warning:** Certificate issuance and renewal validate over HTTP-01, so both LoadBalancers must be reachable from the public internet on port 80. If you need to IP-restrict the dashboard LoadBalancer, coordinate a DNS-01 solver with support first. Otherwise renewals fail silently and the certificate expires.

---

## Get the Manifests

> **Note:** Cloning the releases repo and pulling the `ghcr.io/agenteye-enterprise/*` images both require signed enterprise access. See [GitHub token setup](/agenteye/github-token) to obtain your token.
...
> **Tip:** The server's readiness probe already reflects Postgres + ClickHouse health, and `third-party/robusta/` adds opt-in Kubernetes-native pod-failure alerting to Slack. See [Health monitoring](/agenteye/health-monitoring).
...
> **Note:** Two allowlist mechanisms ship for this instance. This guide uses `values-dashboard.yaml`, which restricts access with the portable `service.loadBalancerSourceRanges` field. A parallel `values-internal.yaml` is also provided for AWS environments that prefer the `service.beta.kubernetes.io/aws-load-balancer-source-ranges` annotation instead. Pick one and use it consistently; the steps below assume `values-dashboard.yaml`.
...
**If it fails:** `<pending>` after 10 minutes usually means the cloud provider can't provision a LoadBalancer. Both `traefik-public` and `traefik-dashboard` are external LoadBalancers, so check the usual provisioning blockers: subnet tags (EKS requires `kubernetes.io/role/elb`), VPC configuration, and LoadBalancer service quotas.
...
This secret authenticates with `ghcr.io` to pull the AgentEye container images. See [GitHub token setup](/agenteye/github-token) for how to generate your PAT.
...
**If it fails:** `ErrImagePull` or `401 Unauthorized` means the PAT is invalid or lacks `read:packages` scope. Re-check [GitHub token setup](/agenteye/github-token).
...
> **Warning:** We use `-hex` (not `-base64`) to generate the password. Base64 output can contain `+`, `/`, and `=` which break the `DATABASE_URL` connection string. See [Troubleshooting](/agenteye/troubleshooting) for details.

> **Note:** Store `POSTGRES_PASSWORD` in your secrets manager immediately. You will need it if you ever restore from a backup or connect to the database directly.
...
The admin key is the bootstrap credential. The server upserts it on every startup with all permissions. Use it to create scoped collector keys in Phase 7. See [API keys](/agenteye/api-keys) for the full permissions model.

> **Note:** Store `ADMIN_KEY` in your secrets manager immediately.
...
> **Note:** Store the SMTP credentials in your secrets manager.
...
> **Warning:** Store `ORG_CH_SECRET` in your secrets manager and don't rotate it casually.
...
**If it fails:** The most common cause is a `POSTGRES_PASSWORD` containing URL-unsafe characters that break the `DATABASE_URL`. See [Troubleshooting](/agenteye/troubleshooting).
...
For full pod wiring (SecretProviderClass, IRSA setup, rotation behavior, troubleshooting) see [Single-pod deployment](/agenteye/single-pod-deployment).
...
See [Collector installation](/agenteye/collector-installation) for complete collector setup including Kubernetes volume mounts.
...
See [API keys](/agenteye/api-keys) for the full key management reference.
...
> **Note:** Complete §2.6 first. `org create` refuses to run while the server is still on the built-in dev `ORG_CH_SECRET`, and the per-org ClickHouse user it provisions depends on that secret being strong and stable.
...
Builtin permission sets are `admin`, `standard`, and `read-only`. **Per-org API keys are still minted in the dashboard/API by org members (§7.2 shows the keys API); only the org + member lifecycle is operator-only.** Full reference and a worked example: [Tenant management](/agenteye/tenant-management).
...
A single `agenteye-backup` CronJob runs daily at 03:00 UTC. It dumps **both** stores: PostgreSQL (relational state) and ClickHouse (the `events` + `evaluations` analytics tables), then streams them as one compressed archive straight to the object storage you wire in your overlay. The `.tar.gz` is piped directly to the upload command and is never written to the pod's disk.
...
The backup CronJob streams the compressed archive straight to object storage, so the shipped pipeline ends in an AWS S3 upload that reads from **stdin**. The base sets a default `BACKUP_BUCKET` you must override with your own bucket. The pipeline runs under `set -euo pipefail`, so a broken pipe aborts the job instead of reporting a false success.

**On AWS:** set `BACKUP_BUCKET` to your bucket and grant the `agenteye-backup` ServiceAccount write access via IRSA (no script change needed). The shipped upload pipeline is:

```bash
tar c -C "${DIR}" . | pv -f -i 15 -s "${DUMPS_BYTES}" | gzip \
  | aws s3 cp - "s3://${BACKUP_BUCKET}/${FILENAME}"
```

**On GCP / Azure:** swap only the final stage (`aws s3 cp -`) for the provider equivalent below. Replace it (don't append) because a leftover `aws s3 cp` stage under `set -euo pipefail` fails the job. Each equivalent reads the archive from stdin, matching the streaming pipeline; there is no on-disk `/tmp/${FILENAME}` to point at, because the archive is never written to disk.

| Cloud | Final pipeline stage (reads stdin) |
|---|---|
| **AWS S3** | `aws s3 cp - "s3://${BACKUP_BUCKET}/${FILENAME}"` (ships by default) |
| **GCP Cloud Storage** | `gsutil cp - "gs://${BACKUP_BUCKET}/${FILENAME}"` |
| **Azure Blob** | `az storage blob upload --file /dev/stdin -c backups -n "${FILENAME}"` |
...
The raw dumps are buffered in a 30 GiB `emptyDir` at `/tmp`; the ClickHouse `events` dump dominates its size. If your data is larger, raise the volume's `sizeLimit` in your overlay.
...
| `AGENT_API_KEY` | `agenteye-agent` secret (optional) | Seeds the protected `dashboard-assistant` key the AI assistant's agent service uses (the same value the agent reads as `AGENTEYE_API_KEY`). Unset means the key isn't seeded and the assistant stays off. See [AI assistant](/agenteye/assistant). |
| `AGENTEYE_AGENT_URL` | Deployment manifest | `http://agent:9100` -- where the **server** reaches the agent service to run the audit's agentic investigation (the same connection the dashboard uses). Unset ⇒ audits run policy-only. See [AI assistant](/agenteye/assistant). |
...
| `EVALUATOR_ENDPOINT`, `EVALUATOR_TOKEN` | `agenteye-evaluator` secret (optional) | Wire the evaluation pipeline. With the secret absent the pipeline is a no-op. See [Evaluation suite](/agenteye/evaluation-suite). |
...
| `AGENTEYE_AGENT_TOKEN` | `agenteye-agent` secret (optional) | Shared secret the dashboard presents to the AI assistant; the assistant stays inert until its LLM endpoint is configured. See [AI assistant](/agenteye/assistant). |
...
```mermaid
flowchart LR
    subgraph collectors["Collector Clusters"]
        COL["Collector Pod<br/>+ client cert + API key"]
    end
    BR["Browser (VPN)<br/>Developer"]
    subgraph ae["AgentEye Cluster"]
        TP["Traefik Public (mTLS)"] --> SRV["Server"] --> PG[("PostgreSQL")]
        TD["Traefik Dashboard"] --> DASH["Dashboard"]
    end
    COL -->|"POST /events, 443"| TP
    BR -->|"HTTPS, 443"| TD
```
...
For a comprehensive list of common issues and solutions, see [Troubleshooting](/agenteye/troubleshooting).
...
- [Collector installation](/agenteye/collector-installation): start sending events from your agents into the cluster you just deployed.
- [API keys](/agenteye/api-keys): mint scoped keys for collectors and dashboard users.
- [Tenant management](/agenteye/tenant-management): provision isolated organizations if you run multiple teams.
- [Troubleshooting](/agenteye/troubleshooting): resolve pod, certificate, or backup issues.
