---
title: "Queries"
description: "AgentEye Queries documentation."
---


Ask any question of your agent data and get an answer in seconds. AgentEye gives you a library of saved, ready-to-run queries over your events and evaluations, so you start from a working example instead of a blank SQL editor.
...
- **Read-only by design.** Queries run against your analytics store (ClickHouse) and are validated on the server: only `SELECT` and `WITH` statements are allowed, with a statement timeout and a row cap. An exploratory query can never modify your data, and a runaway one gets stopped for you.
