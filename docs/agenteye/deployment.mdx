...
Run AgentEye on your own infrastructure, so your agent data never leaves it. AgentEye is a self-hosted platform for observing, evaluating, and improving your AI agents in production. Use this page for the full picture (the components, the datastores they need, and every configuration knob) plus a copy-paste Docker Compose quick start.

> **Tip:** In a hurry? Skip to the [Docker Compose quick start](#docker-compose-recommended). It brings up Postgres, the server, and the dashboard with one command.

## Prerequisites

Before you start, make sure you have:

- **PostgreSQL 15+**: the relational store. 15+ is required; see [Architecture overview](#architecture-overview).
- **ClickHouse 24+**: the analytics store for every ingested event.
- **A host for the containers**: Docker with Compose, or a Kubernetes cluster.
- **Object storage** (S3, GCS, or Azure Blob): only if you want automated backups.
- **Signed enterprise access**: the Docker images live in a private registry (`ghcr.io/agenteye-enterprise/*`); pulling them needs a signed enterprise credential.

Redis 7+ is optional (a shared cache and rate-limit backend); AgentEye runs fine without it.

## Choose your path

This page covers the general Docker and Docker Compose deployment plus the complete configuration reference. For a more opinionated route, start with one of these instead:

- **[Single-pod deployment](/agenteye/single-pod-deployment)**: the whole stack in one pod for a fast evaluation.
- **[Kubernetes deployment](/agenteye/kubernetes-deployment)**: production-grade manifests with backups and scaling.
- **[Managed deployment](/agenteye/managed-deployment)**: let Exosphere run it for you.
...
```mermaid
flowchart LR
    subgraph agents["AI agent machines"]
        SDK["Python SDK<br/>writes JSONL"] --> COL["agenteye-collector"]
    end
    subgraph infra["Your infrastructure"]
        DASH["Dashboard"] -->|API| SRV["Server"]
        SRV <--> PG[("PostgreSQL 15+<br/>relational store")]
        SRV <--> CH[("ClickHouse 24+<br/>events / analytics")]
        SRV -.-> RDS[("Redis 7+<br/>optional")]
    end
    COL -->|HTTP| SRV
```
...
- **Postgres 15+**: REQUIRED. Upgrade before deploying this version (the multi-tenant schema uses a Postgres 15+ feature and will not run on 14). Stores the transactional (OLTP) tables the app reads and writes constantly: `api_keys`, `users`, `sessions`, `evaluation_jobs` (queue), `dashboards`, `saved_queries`, `otp_codes`, plus the multi-tenant tables `orgs`, `org_memberships`, `org_settings`.
- **ClickHouse 24+**: REQUIRED. The analytics store for every ingested event. Engine: `ReplacingMergeTree`, partitioned by month, ordered by `(session_id, ts, dedup_key)`. The server connects via `CLICKHOUSE_URL`; the bundled `deploy/base/clickhouse/` ships a perf-tuned single-node configuration. **Multi-tenant requirement:** the bundled config enables SQL access management + `users_without_row_policies_can_read_rows=false` so the server can create one read-only ClickHouse user + row policy per organization (the engine-enforced isolation boundary for the SQL editor and the AI assistant). If you supply your own ClickHouse config, carry these settings over (see `deploy/base/clickhouse/configmap.yaml`).
...
> **Note:** The images live in a private registry. Pulling them needs signed enterprise access: a `AGENTEYE_TOKEN` credential you receive as a signed customer. If you are still evaluating, skip ahead and read on; the configuration reference below applies once you have access.
...
> **Note:** Current builds publish under `beta-latest`; `latest` is assigned only to stable releases. For production, pin a specific `:v<version>` tag; see [Available Image Tags](#available-image-tags).

### Environment variables

> **Note:** Minimum required to boot: `DATABASE_URL` + `CLICKHOUSE_URL`. Everything else in the table below is optional and has a sensible default. Set only what you need.

| Variable | Required | Default | Description |
|---|---|---|---|
| `DATABASE_URL` | Yes | none | Postgres connection string in standard libpq DSN format (the ordinary `postgres` URL scheme your Postgres client already uses). Supports `?sslmode=require` and other libpq parameters. The secret in it must not contain `/`, `+`, or `=`; use `openssl rand -hex` to generate a URL-safe one. |
...
| `AGENTEYE_AGENT_URL` / `AGENTEYE_AGENT_TOKEN` | No | none | The audit's agentic investigation calls the AI-assistant `agent` service, **reusing the same connection as the assistant**. Set these two on the **server** as well (the bundled manifests/compose do). Both set ⇒ audits run the AI investigation; either unset ⇒ audits run **policy-only** (the deterministic SQL policy pass still runs), regardless of the per-audit `llm_enabled` flag. The agent must also have an LLM configured. See the [AI assistant](/agenteye/assistant) guide. |
...
| `AGENTEYE_AGENT_URL` | No | none | Base URL of the optional AI-assistant `agent` service, e.g. `http://agent:9100`. **Leave it unset to hide the assistant entirely**: no assistant bubble appears in the dashboard. See the [AI assistant](/agenteye/assistant) guide. |
| `AGENTEYE_AGENT_TOKEN` | No | none | Shared secret the dashboard presents to the `agent` service. Must match the `AGENTEYE_AGENT_TOKEN` configured on the agent. See the [AI assistant](/agenteye/assistant) guide. |
...
Full setup, the complete environment-variable reference, telemetry options, and the security model are in the **[AI assistant](/agenteye/assistant)** guide.
...
- 256 MiB mark cache + 256 MiB uncompressed cache (sized to coexist with the 4 GiB pod limit)
...
- Query time caps: the default user profile caps at 60s; the read-only profile used for untrusted `/queries/run` caps at 10s
- 100 GiB persistent volume (PVC) attached to the ClickHouse StatefulSet, the Kubernetes controller that gives ClickHouse stable per-pod storage. Customer overlays SHOULD override it to a fast SSD storage class for production.
...
The destination bucket and cloud credentials are configured per overlay. See the **Backups** section of [Kubernetes deployment](/agenteye/kubernetes-deployment) for upload configuration and restore steps.
...
**First login.** Once the stack is up, open the dashboard at `http://localhost:3000`, enter an email that matches `ALLOWED_EMAILS`, and request a one-time code. If you didn't configure SMTP, the code is printed to the server logs. Read it with `docker compose logs server`. Enter the code and you land in the app. For a guided first-run walkthrough, see [Getting started](/agenteye/getting-started).
...
![The per-org settings page, where operational knobs like allowed sign-ins and session lifetime are edited without a restart](/agenteye/images/settings.png)
...
| Default user permissions | `DEFAULT_USER_PERMISSIONS` | The **name** of the permission set preselected when an admin opens **+ new user** (not a list of individual tokens). Defaults to `standard`: read-only access plus the everyday on-call actions (trigger re-evaluations, run queries, ack incidents, use the assistant). See [API keys](/agenteye/api-keys) for the permission sets available. |
...
**Before creating a second org:** set a strong, stable `ORG_CH_SECRET` (the `org create` command refuses to run on the built-in dev default) and ensure Postgres is **15+**. **Unchanged:** per-org API keys are still minted in the dashboard/API by org members; only the org + member lifecycle moved to the CLI. Full command reference and a worked example: **[Tenant management](/agenteye/tenant-management)**.
...
- **Health monitoring**: On Kubernetes the readiness probe uses `/ready` (Postgres + ClickHouse reachability) while liveness stays on `/health`. For fleet-wide "is AgentEye itself up?" alerting to Slack, enable the opt-in Robusta add-on; see [Health monitoring](/agenteye/health-monitoring).
...
- [Getting started](/agenteye/getting-started): send your first events and confirm they land in the dashboard.
- [Collector installation](/agenteye/collector-installation): install `agenteye-collector` on your agent machines.
- [Kubernetes deployment](/agenteye/kubernetes-deployment): production manifests, backups, and scaling.
- [Tenant management](/agenteye/tenant-management): provision additional organizations on one deployment.
