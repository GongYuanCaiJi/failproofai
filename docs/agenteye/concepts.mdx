---
title: "Concepts"
description: "AgentEye Concepts documentation."
---


This page defines the vocabulary AgentEye uses. If a term in another guide is unfamiliar, it's defined here. You don't need to read it end to end: skim it, or jump back when you hit a word you want pinned down.
...
**Agent**
A named actor inside a run, identified by an `agent_id`. A run can involve several agents: a planner that spawns a summarizer sub-agent, for example. Sub-agents carry a `parent_id`, which is what lets AgentEye draw them on their own lanes in the execution graph.
...
**Context-window fill**
The percentage of a model's context window a response consumed. AgentEye stamps it on `model_response` events for models it recognizes, so prompt growth and impending compaction are visible right in the event stream.
...
**Evaluator**
Your scoring service. AgentEye POSTs a finished run's transcript to it and stores the scores it returns. AgentEye does not ship a default evaluator; the scoring logic is yours.
...
**Finding**
One ranked, evidence-backed result from an audit run. A finding names a pattern, links to the exact sessions behind it, and carries a triage lifecycle (acknowledge, resolve, mute, dismiss). AgentEye deduplicates findings run-over-run so a known pattern updates instead of piling up.
...
**Organization (tenant)**
An isolated workspace. One AgentEye instance can host many organizations, each with its own users, keys, and data. Every dashboard URL is scoped under your org slug (`/<org-slug>/…`). See [Tenant management](/agenteye/tenant-management).

**Collector**
`agenteye-collector`, the lightweight daemon that runs on each agent machine, batches the events the SDK writes to disk, and ships them to the server. See [Collector installation](/agenteye/collector-installation).
...
**Server**
The ingest and API service. It receives events, writes them to ClickHouse, keeps operational state in PostgreSQL, and serves the dashboard and CLI.
...
- [Getting started](/agenteye/getting-started): put the concepts to work end to end.
