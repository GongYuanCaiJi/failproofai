...
failproofai est livré avec 39 politiques intégrées qui détectent les modes de défaillance courants des agents. Chaque politique se déclenche sur un type d'événement hook spécifique et un nom d'outil. Dix-neuf politiques acceptent des paramètres permettant d'ajuster leur comportement sans écrire de code. Cinq politiques de workflow imposent un pipeline commit → push → PR → CI avant que Claude ne s'arrête.
...
| [Commandes d'infra](#infra-commands) | block-kubectl, block-terraform, block-aws-cli, block-gcloud, block-az-cli, block-helm, block-gh-pipeline | PreToolUse |
| [Secrets (assainisseurs)](#secrets-sanitizers) | sanitize-jwt, sanitize-api-keys, sanitize-connection-strings, sanitize-private-key-content, sanitize-bearer-tokens | PostToolUse |
...
- **`sanitize-`** — supprime les données sensibles de la sortie de l'outil avant que l'agent ne la voie.
...
Dans votre configuration, vous pouvez faire référence à une politique intégrée par son nom court ou son nom qualifié ; les deux formes désignent la même politique :
...
<Tip>
Toutes les politiques prennent en charge un champ optionnel `hint` dans `policyParams`. Ce hint est ajouté au message deny ou instruct que Claude reçoit, fournissant des conseils concrets sans modifier le code de la politique. Fonctionne avec les politiques intégrées, personnalisées et de convention. Voir [Configuration → hint](/fr/configuration#hint-cross-cutting) pour plus de détails.
</Tip>
...
Bloque les invocations contenant le mot-clé `sudo`. La correspondance de motifs est effectuée sur les tokens de commande analysés, et non sur la chaîne brute, afin d'empêcher les contournements via l'injection d'opérateurs shell.

**Paramètres :**

| Paramètre | Type | Par défaut | Description |
|-------|------|---------|-------------|
| `allowPatterns` | `string[]` | `[]` | Préfixes de commandes exacts autorisés. Chaque entrée est mise en correspondance avec les tokens argv analysés. |
...
<Note>
Les motifs sont mis en correspondance avec les tokens analysés, et non avec la chaîne de commande brute. Cela empêche les contournements via des opérateurs shell ajoutés (ex. `sudo systemctl status x; rm -rf /` ne correspond pas à `sudo systemctl status *`).
</Note>
...
| Paramètre | Type | Par défaut | Description |
|-------|------|---------|-------------|
| `allowPaths` | `string[]` | `[]` | Chemins qu'il est sûr de supprimer récursivement (ex. `/tmp`). |
...
La grammaire des motifs est la même que pour [`block-sudo`](#block-sudo) : les tokens sont mis en correspondance avec l'argv analysé, `*` est un joker pour un token, et toute commande contenant un opérateur shell autonome (`&&`, `||`, `|`, `;`) ou un token avec des métacaractères shell intégrés est rejetée avant la vérification de la liste blanche pour empêcher les injections de contournement.
...
Les sous-commandes `gh` en lecture seule telles que `gh pr view`, `gh pr list`, `gh run list`, `gh release view` et `gh api repos/.../...` **ne sont pas** concernées par cette politique — elles sont régulièrement nécessaires pour les vérifications de workflow (notamment le `require-ci-green-before-stop` de failproofai).
...
Ne bloque pas les fichiers `.envrc` ni les autres fichiers liés à l'environnement — uniquement les fichiers nommés exactement `.env`.
...
Intercepte les opérations SQL destructives avant qu'elles ne s'exécutent contre votre base de données.
...
Détecte les situations où les agents restent bloqués ou se comportent de manière inattendue.
...
Impose un workflow de fin de session discipliné. Ces politiques se déclenchent sur l'événement **Stop** et empêchent Claude de s'arrêter tant que chaque condition n'est pas remplie. Elles suivent une chaîne de dépendances naturelle : commit → push → PR → CI. Si une politique refuse, les politiques suivantes de la chaîne sont ignorées (court-circuit sur refus).
...
**Événement :** Stop  
**Par défaut :** Refuse l'arrêt lorsqu'il y a des modifications non commitées (fichiers modifiés, indexés ou non suivis). Renvoie un message d'information lorsque le répertoire de travail est propre.
...
Exécutez `gh auth login` avec un token d'accès personnel disposant du scope `repo` pour l'accès en lecture aux pull requests. Si `gh` n'est pas installé ou n'est pas authentifié, la politique est fail-open et signale la raison à Claude.
</Note>
...
**Événement :** Stop  
**Par défaut :** Refuse l'arrêt lorsque la branche courante ne peut pas être fusionnée proprement dans la branche de base. La politique vérifie d'abord qu'il existe une PR `OPEN` sur GitHub pour la branche — sans PR ouverte, il n'y a pas de cible de fusion à imposer, donc toute la politique court-circuite vers allow. Une fois une PR `OPEN` confirmée, deux sondes indépendantes s'exécutent :

1. **Locale** — `git merge-tree --write-tree --name-only origin/<baseBranch> HEAD`. En cas de conflit, le message de refus nomme les fichiers en conflit afin que Claude sache exactement quoi résoudre.
2. **GitHub** — réutilise le résultat `gh pr view --json mergeable,state` déjà récupéré lors de la pré-vérification. Détecte les conflits qu'un `origin/<baseBranch>` local obsolète manquerait (ex. quelqu'un a fusionné une PR conflictuelle sur `main` depuis le dernier fetch). Un résultat `CONFLICTING` entraîne un refus. Un résultat `UNKNOWN` entraîne également un refus et demande à Claude d'attendre environ 10 secondes et de re-vérifier avant de tenter de s'arrêter à nouveau — cela prévient les faux négatifs pendant que GitHub recalcule.

Ignore entièrement (allow) lorsque : `gh` n'est pas installé, aucune PR n'existe pour la branche, l'état de la PR n'est pas `OPEN` (ex. `MERGED`, `CLOSED`), ou `gh pr view` renvoie une sortie non analysable. Également fail-open lorsque `origin/<baseBranch>` est absent localement ou qu'il n'y a aucun commit en avance sur la base — ces replis de couche 1 consultent tout de même la fusionnabilité PR mise en cache avant d'autoriser.
...
**Événement :** Stop  
**Par défaut :** Refuse l'arrêt lorsque les vérifications CI échouent ou sont encore en cours sur la branche courante. Vérifie à la fois les exécutions de workflows GitHub Actions et les vérifications de bots tiers (ex. CodeRabbit, SonarCloud, Codecov). Traite les conclusions `skipped` et `cancelled` comme des succès. Renvoie un message d'information lorsque toutes les vérifications réussissent.

Aucun paramètre.

<Note>
Cette politique nécessite que [GitHub CLI](https://cli.github.com/) (`gh`) soit installé et authentifié.
Exécutez `gh auth login` avec un token d'accès personnel disposant du scope `repo` pour l'accès en lecture aux exécutions de workflows Actions et à l'API Checks. Si `gh` n'est pas installé ou n'est pas authentifié, la politique est fail-open et signale la raison à Claude.
</Note>
