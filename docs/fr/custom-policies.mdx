...
Les politiques personnalisées vous permettent d'écrire des règles pour n'importe quel comportement d'agent : appliquer des conventions de projet, prévenir la dérive, bloquer les opérations destructrices, détecter les agents bloqués, ou s'intégrer à Slack, des workflows d'approbation, et bien plus encore. Elles utilisent le même système d'événements de hook et les mêmes décisions `allow`, `deny`, `instruct` que les politiques intégrées.
...
Déposez des fichiers `*policies.{js,mjs,ts}` dans `.failproofai/policies/` et ils sont chargés automatiquement — aucun flag ni modification de configuration nécessaire. Cela fonctionne comme les hooks git : déposez un fichier, ça marche tout seul.
...
- Les répertoires projet et utilisateur sont tous deux analysés (union — pas de premier-scope-gagne)
...
<Tip>
Les politiques de convention sont le moyen le plus simple d'établir un standard de qualité pour votre organisation. Versionnez `.failproofai/policies/` dans git et chaque membre de l'équipe obtient automatiquement les mêmes règles — aucune configuration individuelle requise. Au fur et à mesure que votre équipe découvre de nouveaux modes d'échec, ajoutez une politique et poussez. Au fil du temps, celles-ci deviennent un standard de qualité vivant qui s'améliore à chaque contribution.
</Tip>
...
Le chemin absolu résolu est stocké dans `policies-config.json` sous `customPoliciesPath`. Le fichier est rechargé à chaque événement de hook — aucune mise en cache entre les événements.
...
`allow(message)` autorise l'opération **et** envoie un message informationnel à Claude. Le message est délivré comme `additionalContext` dans la réponse stdout du gestionnaire de hook — le même mécanisme utilisé par `instruct`, mais sémantiquement différent : c'est une mise à jour de statut, pas un avertissement.
...
- **Accumulation de messages multiples :** si plusieurs politiques retournent chacune `allow(message)`, tous les messages sont joints avec des sauts de ligne et délivrés ensemble
...
| Fichier introuvable | Avertissement journalisé dans `~/.failproofai/hook.log` ; les intégrées continuent |
...
| `fn` lève une exception au moment de l'exécution | Erreur journalisée ; ce hook est traité comme `allow` ; les autres hooks continuent |
...
## Exemple complet : politiques multiples
...
Aucune commande d'installation nécessaire — les fichiers sont détectés automatiquement au prochain événement de hook.