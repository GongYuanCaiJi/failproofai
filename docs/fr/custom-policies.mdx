...
Les politiques personnalisées vous permettent d'écrire des règles pour n'importe quel comportement d'agent : appliquer des conventions de projet, prévenir la dérive, contrôler les opérations destructives, détecter les agents bloqués, ou s'intégrer à Slack, des workflows d'approbation, et bien plus encore. Elles utilisent le même système d'événements de hook et les mêmes décisions `allow`, `deny`, `instruct` que les politiques intégrées.
...
Déposez des fichiers `*policies.{js,mjs,ts}` dans `.failproofai/policies/` et ils sont chargés automatiquement — aucun indicateur ni modification de configuration nécessaire. Cela fonctionne comme les hooks git : déposez un fichier, il s'applique immédiatement.
...
- Fonctionne en parallèle avec les politiques explicites `--custom` et les politiques intégrées

<Tip>
Les politiques par convention sont la façon la plus simple d'établir un standard de qualité pour votre organisation. Versionnez `.failproofai/policies/` dans git et chaque membre de l'équipe obtient automatiquement les mêmes règles — sans configuration individuelle requise. Au fur et à mesure que votre équipe découvre de nouveaux modes d'échec, ajoutez une politique et poussez. Au fil du temps, celles-ci deviennent un standard de qualité vivant qui s'améliore à chaque contribution.
</Tip>
...
# Supprimer le chemin des politiques personnalisées de la config
failproofai policies --uninstall --custom
```

Le chemin absolu résolu est stocké dans `policies-config.json` sous `customPoliciesPath`. Le fichier est rechargé à chaque événement de hook — il n'y a aucune mise en cache entre les événements.
...
| `instruct(message)` | Ajouter du contexte sans bloquer | Donner à l'agent un contexte supplémentaire pour rester sur la bonne voie |
...
`allow(message)` autorise l'opération **et** envoie un message informatif à Claude. Le message est transmis en tant que `additionalContext` dans la réponse stdout du gestionnaire de hook — le même mécanisme que `instruct`, mais sémantiquement différent : c'est une mise à jour de statut, pas un avertissement.
...
Aucune commande d'installation n'est nécessaire — les fichiers sont automatiquement pris en compte au prochain événement de hook.