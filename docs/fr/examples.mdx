---
title: Exemples
description: "Comment configurer les hooks pour Claude Code et l'Agents SDK"
icon: book-open
---
...
## Configurer les hooks pour Claude Code
...
<Steps>
  ...
  <Step title="Lancer Claude Code">
    ...
    Les politiques s'exécutent désormais automatiquement à chaque appel d'outil. Essayez de demander à Claude de lancer `sudo rm -rf /` — la commande sera bloquée.
  </Step>
</Steps>

---

## Configurer les hooks pour l'Agents SDK

Si vous développez avec l'[Agents SDK](https://docs.anthropic.com/en/docs/agents-sdk), vous pouvez utiliser le même système de hooks de façon programmatique.
...
<Steps>
  ...
  <Step title="Configurer les hooks dans votre agent">
    Passez les commandes de hook lors de la création de votre processus agent. Les hooks se déclenchent de la même manière que dans Claude Code — via du JSON stdin/stdout :
...
Rappelez aux agents d'exécuter les tests avant de commiter.
...
Chaque membre de l'équipe qui a failproofai installé récupérera automatiquement ces règles.

---

## Construire un standard qualité à l'échelle de l'organisation avec des politiques de convention

La configuration la plus impactante : commitez `.failproofai/policies/` dans votre dépôt avec des politiques adaptées à votre projet. Chaque membre de l'équipe les obtient automatiquement — sans commandes d'installation ni modifications de configuration.
...
<Steps>
  ...
  <Step title="Continuer à améliorer">
    À mesure que votre équipe rencontre de nouveaux cas d'échec, ajoutez des politiques et poussez-les. Tout le monde reçoit la mise à jour au prochain `git pull`. Ces politiques deviennent un standard qualité vivant qui évolue avec votre équipe.
  </Step>
</Steps>
...
|---------|-------------------|
| `policies-basic.js` | Politiques de démarrage — bloquer les écritures en production, les force-push, les scripts pipés |
