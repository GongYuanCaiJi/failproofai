...
<Steps>
  ...
  <Step title="Configurer les hooks dans votre agent">
    Transmettez les commandes de hook lors de la création de votre processus agent. Les hooks se déclenchent de la même façon que dans Claude Code — via des JSON en stdin/stdout :
...
La configuration la plus impactante : commitez `.failproofai/policies/` dans votre dépôt avec des politiques adaptées à votre projet. Chaque membre de l'équipe les obtient automatiquement — sans commandes d'installation, sans modification de configuration.
...
| `policies-advanced/index.js` | Imports transitifs, hooks asynchrones, nettoyage de sortie PostToolUse, gestion de l'événement Stop |