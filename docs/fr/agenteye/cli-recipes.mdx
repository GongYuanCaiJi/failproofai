...
description: "Modèles de requêtes prêts à copier-coller et recettes jq qui transforment les données de sessions, d'événements et d'évaluations en quelque chose qu'un script ou un agent de codage peut automatiser."
---
...
Les modèles ci-dessous sont prêts à copier-coller pour le CLI Failproof AI Observability (`agenteye`). Pour l'installation, l'authentification et la liste complète des options, consultez [CLI](/fr/agenteye/cli) ; exécutez `agenteye -h` ou `agenteye <command> -h` pour l'aide intégrée.
...
2. **Passez `--json` chaque fois que vous analysez la sortie.** Les données vont sur **stdout** en JSON ; les statuts et erreurs destinés à l'humain vont sur **stderr**, de sorte que stdout reste propre pour être pipé dans `jq`.
3. **Branchez sur le code de sortie**, pas sur le texte de stderr : `0` ok · `1` erreur inattendue · `2` arguments invalides · `3` tableau de bord inaccessible · `4` non connecté ou session expirée · `5` permission manquante · `6` ressource introuvable.
...
`whoami` ne génère jamais d'erreur en cas de session manquante ou expirée ; il rapporte `logged_in:false` à la place, ce qui permet à un agent de vérifier l'état de l'authentification en toute sécurité. (Il peut quand même retourner un code non nul si aucune URL de base n'est définie ou si le tableau de bord est inaccessible.)
...
> **Remarque :** Par défaut, `events` lit un flux rapide sans payload. Chaque événement contient un `summary` d'une ligne calculé par le serveur ainsi que des indicateurs comme `is_error` et le nombre de tokens, mais `payload` est retourné sous la forme `{}`. Pour récupérer le payload brut, ajoutez `--full` (ou `--fields payload`). Le flux complet est plus lent à grande échelle, aussi gardez-le borné : combinez `--full` avec un seul `--session-id`.
...
```bash
# en une seule passe : récupérer jusqu'à 500 lignes en pages de 200
...
Limitez les clés (dans le tableau et dans `--json`) pour réduire ce qu'un agent doit lire.
...
Les noms de champs inconnus sont rejetés (code de sortie `2`) avec la liste des valeurs valides — un moyen simple de découvrir les noms de champs.
...
Si vous appartenez à plusieurs organisations, choisissez le tenant actif à la connexion (il est enregistré) :
...
Une connexion multi-organisations sans `--org` retourne un code non nul et affiche les organisations disponibles.
...
## Gérer un incident de façon non interactive
...
> **Remarque :** Les mutations ignorent automatiquement leur invite de confirmation lorsque `--json` est utilisé ou que stdin n'est pas un TTY, de sorte que les agents ne restent jamais bloqués ; passez `--yes`/`-y` pour l'ignorer explicitement ailleurs.
...
Le `--fields` de chaque commande accepte exactement les noms de champs de son propre élément. L'ensemble diffère entre `sessions` et `evals`, donc un nom valide pour l'un peut être rejeté par l'autre.

## Étapes suivantes

- [CLI](/fr/agenteye/cli) : installation, authentification et référence complète des options pour chaque commande.
- [Compétence agent CLI](/fr/agenteye/cli-skill) : empaquetez ces recettes sous forme de compétence que votre agent de codage peut charger.
- [Clés API](/fr/agenteye/api-keys) : créez et délimitez les clés avec lesquelles le CLI, le SDK et le collecteur s'authentifient.
- [Python SDK](/fr/agenteye/python-sdk) : envoyez des événements dans Failproof AI Observability afin que ces recettes aient des données à interroger.