...
<Tabs>
  ...
  - Historique paginé complet de chaque événement de hook déclenché sur toutes les sessions
...
Lorsque vous exécutez le dashboard en **mode développement** (`npm run dev`) et que vous y accédez depuis un nom d'hôte autre que `localhost` — par exemple, un domaine personnalisé, une IP distante ou une URL tunnelisée — vous pouvez voir un avertissement tel que :
...
<Note>
Ceci ne s'applique qu'au mode développement. Lorsque vous exécutez `failproofai` (mode production), il n'y a pas de websocket HMR ni de problème de ressource dev cross-origin.
</Note>