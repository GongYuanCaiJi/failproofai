...
Les deux sous-systèmes partagent des fichiers de configuration dans `~/.failproofai/` et le répertoire `.failproofai/` du projet, mais ils s'exécutent en tant que processus séparés et communiquent uniquement via le système de fichiers.
...
- Code de sortie : `2`
- La raison est écrite sur stderr (pas stdout)
...
3. Réécrire tous les imports `from "failproofai"` vers le chemin dist réel, afin que `customPolicies` soit résolu vers le même registre `globalThis`.
...
Les politiques personnalisées sont évaluées après toutes les politiques intégrées. Un `deny` d'une politique personnalisée court-circuite quand même les politiques personnalisées suivantes (mais tous les intégrés ont déjà été exécutés à ce stade).
...
Une ligne par politique ayant pris une décision autre que allow. Les décisions d'autorisation ne sont pas journalisées (pour maintenir le fichier de taille raisonnable).
...
│   ├── install-prompt.ts         # Invite interactive de sélection de politique
