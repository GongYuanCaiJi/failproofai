...
Il s'agit de la commande enregistrée dans le fichier `settings.json` de Claude Code par `failproofai policies --install`. Vous ne l'appelez normalement pas directement.

Lit une charge utile JSON depuis stdin, évalue toutes les politiques activées et se termine avec un code indiquant la décision :
