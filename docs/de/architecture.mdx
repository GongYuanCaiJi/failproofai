...
Dieses Dokument erläutert, wie failproofai intern funktioniert: wie das Hook-System Agent-Toolaufrufe abfängt, wie die Konfiguration geladen und zusammengeführt wird, wie Policies ausgewertet werden und wie das Dashboard die Agentenaktivität überwacht.
...
1. **Hook-Handler** – Ein schneller CLI-Subprozess, den Claude Code bei jedem Agentenaufruf eines Tools aufruft. Er wertet Policies aus und gibt eine Entscheidung zurück.
...
Claude Code ruft dann `failproofai --hook PreToolUse` als Subprozess vor jedem Toolaufruf auf und übergibt dabei einen JSON-Payload über stdin.
...
**Instruct (beliebiges Event außer Stop):**
```json
{
  "hookSpecificOutput": {
...
**Stop-Event instruct:**
- Exit-Code: `2`
- Begründung wird auf stderr geschrieben (nicht stdout)
...
`allow(message)` ermöglicht es einer Policy, informationellen Kontext an Claude zurückzusenden, selbst wenn die Operation erlaubt ist. Der Hook-Handler schreibt folgendes JSON auf **stdout** (keine Konfigurationsdatei – das ist die Antwort des Handlers an Claude Code, genauso wie deny- und instruct-Antworten oben):
...
- Wenn mehrere Policies `allow` mit einer Nachricht zurückgeben, werden deren Nachrichten mit Zeilenumbrüchen zu einem einzigen `additionalContext`-String zusammengefügt
...
- `enabledPolicies` – deduplizierte Vereinigungsmenge aller drei Dateien
...
3. Vom Nutzer angegebene Werte über die Schema-Standardwerte legen, um `ctx.params` zu erzeugen.
...
6. Wenn das Ergebnis `instruct` ist, die Nachricht aufnehmen und fortfahren.
7. Wenn das Ergebnis `allow` ist, zur nächsten Policy weitergehen.

Nach dem Ausführen aller Policies:
...
Policies, die `params` akzeptieren, deklarieren ein `PolicyParamsSchema` mit Typen und Standardwerten für jeden Parameter. Der Policy-Evaluator injiziert die aufgelösten Werte in `ctx.params`, bevor er `fn` aufruft. Policy-Funktionen lesen `ctx.params` ohne Null-Absicherung, da Standardwerte immer zuerst angewendet werden.

Pattern-Matching innerhalb von Policies verwendet geparste Befehlstokens (argv), kein einfaches String-Matching. Dies verhindert Umgehungsversuche durch Shell-Operator-Injection (z. B. kann ein Muster für `sudo systemctl status *` nicht durch Anhängen von `; rm -rf /` an den Befehl umgangen werden).
...
`src/hooks/custom-hooks-loader.ts` lädt die Policy-Datei des Nutzers:
...
app/
  ...
  project/[name]/session/
    [sessionId]/page.tsx      ← Server-Komponente: Session-Viewer rendern
  ...
  api/
    download/[project]/[session]/route.ts   ← Pro-CLI-Sitzung Export (JSONL oder JSON)
```
...
- Der Session-Viewer parst das JSONL-Transkriptformat von Claude und rendert eine Zeitleiste mit Nachrichten und Toolaufrufen.
...
│   ├── policy-registry.ts        # Policy-Registrierung und -Lookup
