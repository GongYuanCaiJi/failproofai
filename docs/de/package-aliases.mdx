...
description: "Registrierte Aliase zur Typosquatting-Prävention und wie sie funktionieren"
icon: copy
---
...
Um diese Angriffsfläche zu beseitigen, **registrieren wir vorbeugend alle gängigen Schreibvarianten und Formatierungsvarianten** von `failproofai` auf npm. Keiner dieser Namen kann von Dritten registriert werden. Jeder ist ein schlanker Proxy, der das echte `failproofai`-Paket installiert und an dieses delegiert.
...
1. Listet `failproofai` als Abhängigkeit auf – damit wird das echte Paket (einschließlich der `postinstall`-Hook-Einrichtung) bei der Installation ausgeführt
...
Öffnen Sie ein Issue unter [failproofai/failproofai](https://github.com/failproofai/failproofai/issues) und wir registrieren ihn.