...
__tests__/
  hooks/
    ...
    policy-evaluator.test.ts      # Param-Injection und Auswertungsreihenfolge
...
E2E-Tests führen die Binary direkt aus dem Repository-Quellcode aus. Vor dem ersten Durchlauf muss das CJS-Bundle gebaut werden, das Custom-Hook-Dateien verwenden, wenn sie aus `'failproofai'` importieren:
...
`dist/` muss neu gebaut werden, sobald Änderungen an der öffentlichen Hook-API vorgenommen werden (`src/hooks/custom-hooks-registry.ts`, `src/hooks/policy-helpers.ts` oder `src/hooks/policy-types.ts`).
...
__tests__/e2e/
  helpers/
    hook-runner.ts      # Binary starten, Nutzlast-JSON übergeben, Exit-Code + stdout + stderr erfassen
    fixture-env.ts      # Pro-Test isolierte temporäre Verzeichnisse mit Konfigurationsdateien
    payloads.ts         # Claude-konforme Nutzlast-Fabriken für jeden Event-Typ
  hooks/
    ...
    config-scopes.e2e.test.ts      # Config-Zusammenführung über Projekt/Lokal/Global
...
// env.home   - isoliertes Home-Verzeichnis; kein Durchsickern von ~/.failproofai
...
**`Payloads`** – vorgefertigte Nutzlast-Fabriken:
...
Der `forks`-Pool ist wichtig: Thread-basierte Worker teilen sich `globalThis`, was bei Tests mit Subprocess-Erzeugung zu Problemen führen kann. Prozessbasierte Forks vermeiden dies.

---

## CI

Der vollständige CI-Durchlauf (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) muss erfolgreich sein, bevor ein Merge möglich ist. Die E2E-Suite läuft als separater CI-Job parallel dazu.
