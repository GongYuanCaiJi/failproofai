...
| `FAILPROOFAI_HOOK_LOG_FILE` | Percorso personalizzato del file di log degli hook, oppure `true` per quello predefinito (`~/.failproofai/logs/hooks.log`) |

## Telemetria

| Variabile | Descrizione |
|----------|-------------|
| `FAILPROOFAI_TELEMETRY_DISABLED=1` | Disabilita la telemetria anonima d'uso |
