...
Authentication is opt-in. Policies, the dashboard, the audit command, and every other local feature work exactly the same whether you're signed in or not. The login surface exists so that features that **need** a stable identity (re-audit reminders today, more in the future) have somewhere to anchor.
...
|---|---|---|
| `FAILPROOF_API_URL` | (production URL) | Override the api-server base URL. Useful for local development against a self-hosted api-server. |
